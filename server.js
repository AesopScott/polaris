...
async function handleMessage(ws, raw) {
  ...
  if (type === 'transfer-session') {
    const { sessionId: sourceId } = msg;
    const source = sessions.get(sourceId);
    if (!source) return sendTo(ws, { type: 'error', text: 'Source session not found' });

    let diagText;
    try {
      const diagPath = path.join(LOGS_DIR, `diag-${sourceId}.txt`);
      diagText = fs.existsSync(diagPath) ? fs.readFileSync(diagPath, 'utf8') : '(no diag log available for the source session)';
    } catch (e) {
      diagText = `(error reading diag log: ${e.message})`;
    }

    const continuationPrompt =
      `You are continuing a previous Polaris session. Below is the full diagnostic log of the prior session — its prompts, tool calls, tool results, and progress. Pick up where it left off: address any unanswered question, finish any pending work, or verify and refine work that appeared complete. Do not restate the prior conversation; just continue.\n\n` +
      `--- PRIOR SESSION DIAG (sessionId=${sourceId}, name=${source.name || 'untitled'}) ---\n` +
      diagText +
      `\n--- END DIAG ---`;

    const newId = source.isGpt ? `gpt_${Date.now()}` : (source.isCodex ? `codex_${Date.now()}` : `s_${Date.now()}`);
    const newName = `Transfer: ${source.name || 'Session'}`;
    const newWorkDir = source.workDir || null;
    const newProjectName = source.projectName || null;
    const newModel = source.model || null;
    const newTier = source.tier || 'floor';

    const baseSession = {
      id: newId, name: newName, workDir: newWorkDir, projectName: newProjectName,
      model: newModel, tier: newTier,
      status: 'running', startAt: Date.now(), lastActivityAt: Date.now(), stallCount: 0,
      proc: null, watcher: null, timeout: null,
      lines: [], lastPrompt: continuationPrompt, claudeSessionId: null,
      pendingImages: [], pendingDocs: [], pendingAudio: [],
    };

    if (source.isGpt) {
      sessions.set(newId, { ...baseSession, isChat: true, isGpt: true, gptConversationStarted: false });
      broadcast({ type: 'session-created', sessionId: newId, name: newName, workDir: null, projectName: newProjectName, model: newModel, isChat: true, isGpt: true });
    } else if (source.isCodex) {
      sessions.set(newId, { ...baseSession, isChat: true, isCodex: true, codexThreadId: null });
      broadcast({ type: 'session-created', sessionId: newId, name: newName, workDir: newWorkDir, projectName: newProjectName, model: newModel, isChat: true, isCodex: true });
    } else if (source.isChat) {
      sessions.set(newId, { ...baseSession, isChat: true, chipLabel: source.chipLabel || null, chipColor: source.chipColor || null });
      broadcast({ type: 'session-created', sessionId: newId, name: newName, workDir: newWorkDir, projectName: newProjectName, chipLabel: source.chipLabel || null, chipColor: source.chipColor || null, model: newModel, isChat: true });
    } else {
      sessions.set(newId, { ...baseSession, isChat: false });
      broadcast({ type: 'session-created', sessionId: newId, name: newName, workDir: newWorkDir, projectName: newProjectName, model: newModel });
    }

    broadcast({ type: 'line', sessionId: newId, text: `[transfer] Continuing from session ${sourceId} (${source.name || 'untitled'}). Diag log attached as context.`, role: 'system' });
    saveSessions();

    if (source.isGpt) {
      spawnGptChat(newId, continuationPrompt, newTier);
    } else if (source.isCodex) {
      spawnCodexSession(newId, continuationPrompt, readConfig());
    } else if (source.isChat) {
      spawnChatRouter(newId, continuationPrompt, readConfig());
    } else {
      const effectiveWorkDir = newWorkDir || CHAT_DIR;
      runDirectAgent(newId, continuationPrompt, effectiveWorkDir).catch(err => console.error('[transfer-agent] unhandled error:', err.stack || err.message));
    }
    return;
  }
