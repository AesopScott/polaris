...
## Generated Backlog Contract Fields
...
| `priority` | number (optional) | no |
| `impact` | "minor" \| "standard" \| "major" (optional) | no |
| `status` | "backlog" \| "planned" \| "build-started" \| "build-finished" \| "cba-complete" \| "review-blocked" \| "staged" \| "production" \| "failed-smoke-test" \| "stalled" \| "failed" \| "blocked" \| "on-hold" \| "cancelled" \| "ready" \| "in-progress" \| "complete" \| "pr-reviewed" \| "cba-half-complete" \| "smoke-tested" (optional) | no |
| `dependencies` | number[] (optional) | no |
| `plan` | string (optional, nullable) | no |
| `branch` | string (optional, nullable) | no |
| `pr_url` | string (optional, nullable) | no |
| `proofUnits` | object[] (optional) | no |
| `objective` | object (optional) | no |
| `created_at` | string (optional) | no |
| `completed_at` | string (optional, nullable) | no |
...
| `number` | integer | yes |
| `title` | string | yes |
| `expectedBehavior` | string | yes |
| `proofType` | "smoke-command" \| "manual-script" \| "registry-diff" \| "api-check" \| "ui-check" | yes |
| `exactCommand` | string | yes |
| `expectedInitialFailure` | string | yes |
| `expectedPassingEvidence` | string | yes |
| `waiverGuidance` | string (optional) | no |
...
| `nonGoals` | string[] | yes |
| `proofMap` | Record<string, unknown> | yes |
| `stopConditions` | string[] (optional) | no |

### Enum Values

**BacklogStatus:** `backlog`, `planned`, `build-started`, `build-finished`, `cba-complete`, `review-blocked`, `staged`, `production`, `failed-smoke-test`, `stalled`, `failed`, `blocked`, `on-hold`, `cancelled`, `ready`, `in-progress`, `complete`, `pr-reviewed`, `cba-half-complete`, `smoke-tested`

**ImpactEnum:** `minor`, `standard`, `major`
