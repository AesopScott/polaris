...
/**
 * All valid writeMode values. Any unrecognized value causes _evaluatePolicyCore
 * to fail-closed (deny the write) rather than silently grant access.
 * @type {ReadonlyArray<string>}
 */
const KNOWN_WRITE_MODES = Object.freeze(['read-only', 'project-only', 'extended']);
 ...
 * @property {'any'|'standard'|'restricted'} minimumTrustLevel - Reserved for future enforcement.
 *   Currently not evaluated by _evaluatePolicyCore; all classes in blockedCommandClasses are blocked
 *   regardless of session trust level. Do not rely on this field for enforcement decisions.
...
function _evaluatePolicyCore(action, context, policy) {
  ...
  if (action === 'write' || action === 'edit') {
    // Fail-closed: unrecognized writeMode values are denied, not silently allowed
    if (!KNOWN_WRITE_MODES.includes(policy.writeMode)) {
      const reason = `Write blocked: unrecognized writeMode "${policy.writeMode}"`;
    ...
    // Relative paths resolve against process.cwd() and can silently escape allowedRoots.
    // Callers (server.js, tests) must always supply an absolute path.
    if (!path.isAbsolute(context.filePath)) {
      const reason = `Write blocked: relative filePath "${context.filePath}" is not allowed — callers must supply an absolute path`;
      auditEvent.allowed = false;
      auditEvent.reason = reason;
      return { allowed: false, reason, auditEvent };
    }

    const filePath = path.resolve(context.filePath);
  ...
  if (action === 'bash' || action === 'powershell') {
    ...
    // Write boundary check for shell commands — uses policy.allowedRoots to stay consistent
    // with the write/edit boundary check above (including Obsidian + Downloads in extended mode).
    // NOTE: Detection is limited to unquoted absolute Windows paths; env-var-expanded paths,
    // quoted paths, and indirect invocations (cmd /c, Start-Process) are not detected here.
    // Those are handled at the interactive approval layer in shellPrecheck (server.js).
    if (SHELL_WRITE_VERBS.test(flat)) {
      const absPathRe = /[a-zA-Z]:[\\\/][^\s'">,;|&)>]*/g;
      const roots = (policy.allowedRoots || []).map(r => path.resolve(r).toLowerCase());
      for (const p of (flat.match(absPathRe) || [])) {
        const resolved = path.resolve(p).toLowerCase();
        // Sep-guarded prefix: prevents C:\repoHacked matching root C:\repo
        const inAllowed = roots.some(r => resolved === r || resolved.startsWith(r + path.sep));
        if (!inAllowed) {
          const reason = `Shell command blocked: path "${p}" is outside allowed roots (${policy.allowedRoots.join(', ')})`;
          auditEvent.allowed = false;
          auditEvent.reason = reason;
          return { allowed: false, reason, auditEvent };
        }
...
module.exports = {
  ...
  // SHELL_WRITE_VERBS intentionally not exported — module-private implementation detail
  detectInstallerExe,
  buildDefaultPolicy,
  _evaluatePolicyCore,
};
