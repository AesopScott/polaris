...
async function buildMemoryInjectionBlock(memory, project, query, opts = {}) {
  ...
  try {
    ...
    // Race the Firestore search against a short timeout so a slow or hanging
    // read never blocks session start. Timeout resolves to [] (no memories).
    ...
    return formatMemoryBlock(memories, { cap });
  } catch (e) {
