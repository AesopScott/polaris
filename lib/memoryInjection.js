...
// session turn 1. Extracted from server.js for testability.
//
// Exports:
...
//
// Design contract:
//   • Used by chat turn-1 context and direct-agent turn-1 context.
...
const INJECTION_LOG_QUERY_CAP = 2000;
const INJECTION_LOG_MEMORY_CAP = 500;

function truncateInjectionLogText(value, cap = INJECTION_LOG_QUERY_CAP) {
  const text = String(value || '');
  if (text.length <= cap) return text;
  return text.slice(0, Math.max(0, cap - 3)) + '...';
}

function shouldInjectAgentMemory({
  isDeepSeekQueueMode = false,
  continuationContext = null,
  existingMessageCount = 0,
  session = {},
} = {}) {
  return !isDeepSeekQueueMode
    && !continuationContext
    && existingMessageCount === 0
    && !!session.projectName
    && !session.routineTag
...
module.exports = {
  ...
  buildMemoryInjectionBlockWithTrace,
