...
/**
 * Rank chunks by BM25, optional cosine similarity, and optional file freshness.
 * Freshness is a 90-day exponential decay derived from file mtime.
 * Blend weights: BM25+cosine+freshness = 0.50/0.35/0.15;
 *                BM25+cosine only = 0.60/0.40; BM25+freshness only = 0.80/0.20.
 *
 * @param {{ chunks, idf, avgDocLen, termFreqs }} index
 * @param {string} query
 * @param {Array<Array<number>>|null} embeddings — precomputed chunk embeddings
 * @param {Object<string,number>} [fileMtimes] — filename → mtimeMs from fs.statSync
 * @returns {Array} scored chunks, sorted descending by _score, max TOP_N
 */
function rankChunks(index, query, embeddings, fileMtimes) {
  ...
  const scored = chunks.map((chunk, i) => {
    ...
    const mtime     = fileMtimes && fileMtimes[chunk.file];
    const freshness = mtime ? Math.exp(-(now - mtime) / (90 * 24 * 60 * 60 * 1000)) : 0;

    let score = bm25;
    if (qVec && embeddings && embeddings[i]) {
      const cosine = _cosine(qVec, embeddings[i]);
      score = freshness > 0
        ? bm25 * 0.50 + cosine * 0.35 + freshness * 0.15
        : bm25 * 0.60 + cosine * 0.40;
    } else if (freshness > 0) {
      score = bm25 * 0.80 + freshness * 0.20;
    }

    return { ...chunk, _score: score, _freshness: freshness };
  });
