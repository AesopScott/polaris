...
function decayedStrengthForMemory(data, nowSec = Date.now() / 1000) {
  ...
  const stability = STABILITY_BASE * Math.log((data.accessCount || 0) + 2);
...
async function listDistillationCandidates({ limit = 500 } = {}) {
  const firestore = init();
  if (!firestore) return [];
  try {
    const snapshot = await firestore.collection(COLLECTION)
...
async function applyDistilledMemory({ cluster, synthesis }) {
  ...
  try {
    ...
    for (const memory of cluster.memories) {
      const sourceRef = memory.ref || (memory.id ? firestore.collection(COLLECTION).doc(memory.id) : null);
