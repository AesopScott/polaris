 ...
 */
async function addMemory({ project, content, type, tags = [], sessionId, sessionType, source = 'codex' }) {
...
async function addMemories(memories) {
  ...
  for (const m of memories) {
    if (!m.content || !m.project) continue;
  ...
  try {
    await batch.commit();
    console.log(`[memory] ✅ batch wrote ${ids.length} memories`);
    return ids;
  } catch (e) {
...
async function searchMemories(project, query, limit = 5) {
  ...
  try {
    ...
    const scored = snapshot.docs.map(doc => {
      ...
      const ageDays = lastAccessedSec > 0 ? (now - lastAccessedSec) / 86400 : 999;
...
async function decayMemories() {
  ...
  for (const doc of snapshot.docs) {
    const data          = doc.data();
    const lastSec       = data.lastAccessedAt?.seconds || now;
    const daysSince     = (now - lastSec) / 86400;
    const stability     = STABILITY_BASE * Math.log(data.accessCount + 2);
    const newStrength   = Math.min(MAX_STRENGTH, Math.exp(-daysSince / stability));
...
module.exports = { isReady, addMemory, addMemories, searchMemories, reinforceMemory, decayMemories, getAllMemories };
