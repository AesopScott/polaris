...
HITL_NODES: set of node names where advance_graph() will pause until /signal.
"""
...
def build_node(state: TaskState) -> Dict[str, Any]:
    """HITL gate — executor suspends here until human signals code_done."""
    return {
        "current_node": "build",
        "checkpoint_data": {**state.get("checkpoint_data", {}), "code_done": False},
    }
...
def finish_build_node(state: TaskState) -> Dict[str, Any]:
    ...
    for unit in proof_units:
        ...
        if num not in proof_results and not waived:
            unverified.append(num)

    if unverified:
        # Route back to build for remediation
        return {
            "current_node": "build",
            "checkpoint_data": {
...
def review_node(state: TaskState) -> Dict[str, Any]:
    """HITL gate — executor suspends here until human signals approved/request_changes."""
    return {
        "current_node": "review",
        "review_evidence": {**state.get("review_evidence", {}), "review_started": True},
        "checkpoint_data": {**state.get("checkpoint_data", {}), "review_complete": False},
    }
