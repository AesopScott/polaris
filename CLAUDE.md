...
7. **Commit after every change:** After any file edit or write, immediately commit with a conventional message (feat, fix, refactor, docs, chore, perf, ci). Never leave changes uncommitted. Bump `package.json` version **at delivery time** for source, config, runtime behavior, build, UI, or shipped asset changes. Do not bump `package.json` for notes/documentation-only changes.
